...
# import pywinauto.keyboard as pwa_keyboard

from dataclasses import dataclass
from time import sleep
from PIL import Image
# from keys import press, pressAndHold, pressHoldRelease, release, VK_CODE
...
class ImageUtility:
	...
	@staticmethod
	def find_matches_in_image( img : Image.Image, target : Image.Image, threshold : float | int = 0.6, centered : bool = True ) -> list[Rect]:
		...
		# if centered:
		# 	return [ Rect(x=x + int(w/2), y=y + int(h/2), w=w, h=h) for (x, y) in zip(*loc[::-1]) ]
...
class ApplicationWrapper:
	...
	def locateFileOnWindow( self, filepath : str, centered : bool = False, threshold : float | int = 0.6 ) -> Rect | None:
		target = Image.open( filepath )
		source : Image.Image = self.screenshot_window()
		locations : list[Rect] = ImageUtility.find_matches_in_image( source, target, threshold=threshold, centered=centered )
		if len(locations) == 0:
			return None
		return locations[0]
...
def has_no_ores( ) -> bool:
	rect = wrapper.locateFileOnWindow( get_image_path('ore_limit_0.PNG'), centered=False, threshold=0.95 )
	return rect != None

def has_no_money( ) -> bool:
	rect = wrapper.locateFileOnWindow( get_image_path('default_money.PNG'), centered=False, threshold=0.9 )
	return rect != None

def open_settings( keybind : str = 'c' ) -> None:
	rect = wrapper.locateFileOnWindow( get_image_path('settings_button.PNG'), centered=True, threshold=0.8 )
	if rect != None:
		return
...
def open_layouts( keybind='l' ) -> None:
	rect = wrapper.locateFileOnWindow( get_image_path('layout_1.PNG'), centered=True )
	if rect != None:
		return
	wrapper.handler.type_keys(keybind, set_foreground=False, vk_packet=False)
	sleep(0.2)
...
def move_mouse( x, y ) -> None:
	wrapper.handler.drag_mouse_input((40, 40), (50, 40), button='right')
	wrapper.handler.release_mouse_input(absolute=False, button='right')
...
def load_layout_1( ) -> None:
	open_layouts( )
	rect = wrapper.locateFileOnWindow( get_image_path('layout_1.PNG'), centered=False, threshold=0.5 )
	if rect == None:
...
def press_rebirth( ) -> None:
	...
	rect2 = wrapper.locateFileOnWindow( get_image_path('red_yes.PNG'), centered=True, threshold=0.8 )
	if rect2 != None:
	...
	rect = wrapper.locateFileOnWindow( get_image_path('reborn_button.PNG'), centered=True, threshold=0.8 )
	if rect == None:
